...
exports = module.exports = function(options) {
    ...
    var optionsSchema = Joi.object().keys({
        defaultContentType : Joi.string().valid.apply(this,_.values(config.contentType)),
        swagger : Joi.object.keys({
            title : Joi.string(),
            version : Joi.string().optiona().default(""),
            description : Joi.string().optional().default(""),
            schemes : Joi.array().items(Joi.string().valid.apply(this,config.validSchemes)).min(1),
            host : Joi.string(),
    ...
    func.use = function(app,router) {
        ...
        _.each(router.routes,function (route,index,list) {
            ...
            if(_.isUndefined(route.contentType)) {
                list[index].contentType = func.options.defaultContentType;
            }
        ...
        documentation(app,router.routes,func.options);
    };

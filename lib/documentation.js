...
module.exports = function(app,routes,options) {
    ...
    if(options.host!=="") {
        swaggerJson.host = options.host;
    }

    if(options.schemes!=="") {
        swaggerJson.schemes = options.schemes;
    }

    _.each(routes,function(route){

        route.path = swaggifyPath(route.path);
        if(! swaggerJson.paths[route.path]) {

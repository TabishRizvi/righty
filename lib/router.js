...
function validateRouteMapping(routeMapping) {
    ...
    var schema = Joi.object().keys({
        ...
        contentType : Joi.string().valid(_.values(config.contentType)).optional(),
        handler : Joi.func()
    ...
    if(routeMapping.method!="post" && routeMapping.method!="put" && (! _.isUndefined(routeMapping.validate.body) || _.isUndefined(routeMapping.validate.files))) {

        return "use POST or PUT to send body/files";
    }

    if(! _.isUndefined(routeMapping.validate.files)) {
